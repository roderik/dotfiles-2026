function wtn --description "Open a worktree — auto-detects branch, Linear ticket, GitHub PR, or existing worktree"
    ...
    if test -z "$input"
        # No arg → combined picker: existing worktrees + open PRs
        set -l selection (__wtn_combined_picker $repo_root)
        ...
        switch $kind
            case worktree
                __wtn_existing $repo_root $ref $codex_flag
                return $status
            case pr
                set mode pr
        end
    ...
    switch $mode
        case branch
            __wtn_branch $repo_root $ref $codex_flag
        case linear
            __wtn_linear $repo_root $ref $codex_flag
        case pr
            __wtn_pr $repo_root $ref $codex_flag
    end
end

# ── Combined picker: existing worktrees + open PRs ───────────────────────
function __wtn_combined_picker
    set -l repo_root $argv[1]

    # Build worktree entries (skip main)
    set -l entries
    ...
    # Show picker — display column is the 3rd field

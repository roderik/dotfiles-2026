function otd --description "Delete an opensessions worktree via fzf picker (current preselected)"
    ...
    # Build fzf list: skip main worktree, annotate and sort current first
    set -l current_line
    set -l other_lines
    ...
    end | fzf --height=40% --reverse --header "opensessions: Delete which worktree?" \

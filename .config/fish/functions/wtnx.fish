function wtnx --description "Create worktree from Linear ticket and launch Codex via ACP"
  set -l ticket $argv[1]
  if test -z "$ticket"
  ...
  # Launch Codex via ACP for remote control
  echo ""
  echo "Launching Codex via ACP for $ticket..."
  echo "You can control this session from Telegram with:"
  echo "  /acp steer 'status' or /acp steer 'what are you working on?'"
  echo ""
  acpx codex --cwd "$worktree_path" --label "$ticket" "\$execute $ticket — $title"
end
